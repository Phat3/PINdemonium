...
for line in report_file:
        ...
        if new_dict['runnable?'] == 'PROBABLY YES':
                zero_count = len( dict((key, val) for key, val in json.loads(line).items() if (all(x == '0' for x in val) and key != 'dump number')) )	#filter all the val that is not 0 and get the count of the resulting dict
